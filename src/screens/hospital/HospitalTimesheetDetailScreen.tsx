                ...
                {!myReview.isVisible && (
                  <View style={{ flexDirection: 'row', alignItems: 'center', marginTop: Spacing.md, backgroundColor: Colors.warningLight, borderRadius: BorderRadius.sm, padding: Spacing.sm }}>
                    ...
                    <Text style={[Typography.caption, { color: Colors.warning, marginLeft: Spacing.xs, flex: 1 }]}>
                      Hidden until the doctor also reviews (or {(() => { const d = Math.max(0, Math.ceil((new Date(new Date(myReview.createdAt).getTime() + APP_CONFIG.BLIND_REVIEW_DAYS * 86400000).getTime() - Date.now()) / 86400000)); return `${d} day${d !== 1 ? 's' : ''}`; })()})
                    </Text>
                  </View>
                )}
              </View>
              ...
              <View style={styles.infoCard}>
                ...
                <Button
                  ...
                  onPress={() => (navigation as any).navigate('SubmitReview', {
                    ...
                    entityName: doctor ? `Dr. ${doctor.firstName} ${doctor.lastName}` : 'Doctor',
